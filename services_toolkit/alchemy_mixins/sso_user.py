...
class SSOUserMixin:
    ...
    sso_id = Column(String, nullable=False, index=True)
    latest_session_started_at = Column(
    ...
    def auth_callback(cls: type, token: str):
        ...
        user.update(
            latest_session_request_at=datetime.utcnow().replace(tzinfo=timezone.utc)
        )
    ...
    def latest_session_duration(self) -> Optional[timedelta]:
        if not self.latest_session_request_at and not self.latest_session_started_at:
            return None
        session_duration = (
            self.latest_session_request_at - self.latest_session_started_at
        )
    ...
    def update_latest_session_time(self) -> datetime:
        ...
        if not self.is_online:
            ...
            self.update(
                latest_session_started_at=timestamp
